...
function pngBlob(canvas){return new Promise(function(resolve,reject){canvas.toBlob(function(value){value?resolve(value):reject(new Error('PNG 변환 실패'))},'image/png')})}
async function save(){
  ...
  try{
    ...
    var options={width:width,height:height,canvasWidth:width,canvasHeight:height,windowWidth:width,windowHeight:height,scrollX:0,scrollY:0,scale:1,useCORS:true,allowTaint:false,backgroundColor:getComputedStyle(sheet).backgroundColor||'#ffffff',logging:false,imageTimeout:2500};
    var canvas=await html2canvas(sheet,options);
    var value=await pngBlob(canvas),url=URL.createObjectURL(value),a=document.createElement('a'),name=((state().pairName||'WeLog').trim()||'WeLog');
    a.href=url;a.download=name+'.png';document.body.appendChild(a);a.click();a.remove();setTimeout(function(){URL.revokeObjectURL(url)},1000);
...
function install(){var old=document.getElementById('printBtn');if(!old)return;var button=old.cloneNode(true);old.replaceWith(button);button.dataset.liveSaveReady='16';button.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();save()},true)}
