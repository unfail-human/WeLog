...
async function save(){
  ...
  try{
    ...
    var background=getComputedStyle(sheet).backgroundColor||'#ffffff',rect=sheet.getBoundingClientRect(),fullWidth=Math.ceil(Math.max(sheet.offsetWidth,sheet.scrollWidth)),fullHeight=Math.ceil(Math.max(sheet.offsetHeight,sheet.scrollHeight)),captureX=Math.max(0,Math.floor(rect.left+window.scrollX)),captureY=Math.max(0,Math.floor(rect.top+window.scrollY));
    var options={x:captureX,y:captureY,width:fullWidth,height:fullHeight,windowWidth:Math.ceil(captureX+fullWidth+32),windowHeight:Math.ceil(captureY+fullHeight+32),scrollX:0,scrollY:0,scale:1,useCORS:true,allowTaint:false,backgroundColor:background,logging:false,imageTimeout:2500,removeContainer:true,onclone:function(doc){var copy=doc.getElementById('sheet');if(!copy)return;copy.style.setProperty('width',fullWidth+'px','important');copy.style.setProperty('min-width',fullWidth+'px','important');copy.style.setProperty('max-width',fullWidth+'px','important');copy.style.setProperty('height',fullHeight+'px','important');copy.style.setProperty('transform','none','important');copy.style.setProperty('transform-origin','top left','important');for(var p=copy.parentElement;p&&p!==doc.body;p=p.parentElement){p.scrollLeft=0;p.scrollTop=0;if(p.classList.contains('stage')||p.classList.contains('welog-fit-preview-viewport'))p.style.setProperty('overflow','visible','important')}}};
    var dataUrl;
...
function updateNotice(){var key='welog-update-notice-22';try{if(localStorage.getItem(key))return;localStorage.setItem(key,'1')}catch(e){}setTimeout(function(){alert('업데이트 되었습니다.')},250)}
function install(){var old=document.getElementById('printBtn');if(!old)return;var button=old.cloneNode(true);old.replaceWith(button);button.dataset.liveSaveReady='22';button.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();save()},true)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){install();updateNotice()});else{install();updateNotice()}setTimeout(install,500);
})();
