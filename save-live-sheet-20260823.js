...
async function save(){
  ...
  var label=button.textContent,hidden=[];
  ...
  try{
    ...
    var options={scale:1,useCORS:true,allowTaint:false,backgroundColor:background,logging:false,imageTimeout:2500,removeContainer:true};
    var dataUrl;
  ...
  finally{hidden.forEach(function(x){x[0].style.visibility=x[1]});button.disabled=false;button.textContent=label}
}
function install(){var old=document.getElementById('printBtn');if(!old)return;var button=old.cloneNode(true);old.replaceWith(button);button.dataset.liveSaveReady='20';button.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();save()},true)}
